...
(defgeneric btree-node-insert (tree node key &key value left-node right-node &allow-other-keys)
...
parent node with :left-node value of NODE and a :right-node value of
the new node."))

(defgeneric btree-replace-root (tree &key key value left-node right-node)
  (:documentation "Called in the process of insertion if it is necessary to split the
root."))

(defgeneric btree-node-insert-at-position (tree node position key &key value left-node right-node &allow-other-keys)
...
(defgeneric btree-node-split (tree node &key key value left-node right-node)
...
(defgeneric btree-node-child-at-position (tree node position leftp)
...
(defmethod btree-node-rootp (tree node)
  "EQL test is default."
...
(defmethod btree-node-insert (tree node key &key value left-node right-node &allow-other-keys)
  (if (not (btree-node-fullp tree node))
	...
	(btree-node-insert-at-position tree node position key
      ...
      (multiple-value-bind (left-node right-node median-key median-value)
	  (btree-node-split tree node key)
	(if (not (btree-node-rootp tree node))
	    (btree-node-insert tree (btree-node-parent tree node) median-key
				:value median-value :left-node left-node :right-node right-node)
	    ;; make new root
...
3. Otherwise the leaf node is split into two nodes.
  ...
  (multiple-value-bind (node position)
    ...
    (if (not (btree-node-fullp tree node))
	(btree-node-insert-at-position tree node position key :value value)
	(btree-node-insert tree node key :value value))))

(defgeneric btree-position-for-key (tree key &rest rest &key maxp &allow-other-keys)
  (let* ((node (btree-root tree))
	 (position (apply #'btree-node-position-for-key tree node key rest)))
    (loop :until (btree-node-leafp node)
	  :do (setf position (apply #'btree-node-position-for-key tree node key rest)
		    node (btree-node-child-at-position tree node position t
	  :finally (values node position))))
	 
    
  


;;;; The following is a concrete implemantation of a btree that conforms to the above protocol
(defclass my-node ()
  ((keyvals :initarg :keyvals :initform nil :accessor node-keyvals)
   (children :initarg :childre :initform nil :accessor node-children)
   (parent :initarg :parent :initform nil :accessor node-parent)))

(defclass my-btree ()
  ((root :initarg :root :initform nil :accessor root)
   (maxkeys :initarg :maxkeys :initform 6 :accessor maxkeys)))

(defmethod btree-root ((tree my-btree))
  (root tree))

(defmethod btree-nodep ((tree my-btree) object)
  (typep object 'my-node))

(defmethod btree-node-leafp ((tree my-btree) node)
  (null (node-children node)))

(defmethod btree-node-fullp ((tree my-btree) node)
  (eql (length (node-keyvals node)) (maxkeys tree)))

(defmethod btree-replace-root ((tree my-btree) &key key value left-node right-node)
  (let ((new-root (make-instance 'my-node
				 :elements (list (cons key value))
				 :keyvals (list left-node right-node))))
    (setf (root tree) new-root
	  (node-parent left-node) new-root
	  (node-parent right-node) new-root)
    new-root))

(defmethod btree-node-insert-at-position ((tree my-btree) node position key &key value left-node right-node)
  ;; splice the node in at that position
  (let ((keyvals (concatenate 'list
			       (subseq (node-keyvals node) 0 position)
			       (list (cons left-node right-node))
			       (subseq (node-keyvals node) position)))
	(children (concatenate 'list
			       (subseq (node-children node) 0 position)
			       (list 
			       (subseq (node-children node) position)))
    (setf (node-elements node) elements)))

(defmethod btree-node-split ((tree my-btree) node  &key key value left-node right-node)
  nil)

(defmethod btree-node-position-for-key ((tree my-btree) node key &key maxp &allow-other-keys)
  (declare (ignore maxp))
  (or (position-if #'< (node-elements node) :from-end t)
      0))

(defmethod btree-node-child-at-position ((tree my-btree) node position leftp)
  (if (= (length (node-elements node)) position)
      (progn (assert leftp)
  (let ((elem 
  (destructuring-bind (key value left right)
      (elt (node-elements node) position)
