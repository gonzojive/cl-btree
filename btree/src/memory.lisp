...
(defun sanity-check-btree (tree)
  ...
  (labels ((check-node  (true-parent node)
	     (assert (eql (node-parent node) true-parent))
	     (if (not (btree-node-leafp tree node))
		 (every (curry #'check-node node) (node-children node))
		 t)))
    (check-node nil (btree-root tree))))
...
(defmethod btree-map ((tree memory-btree) map-fn key &key start end value from-end &allow-other-keys)
